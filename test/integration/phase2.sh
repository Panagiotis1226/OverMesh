...
# -relay=false: this test asserts pure holepunching semantics, including
# the clean no-path failure for symmetric NATs (phase3.sh covers relay).
ns $INET "$BIN/overmesh-server" -http "$HTTP" -grpc "$GRPC" -stun "$INET_IP:3478" \
  -relay=false -state-dir "$WORK/server" -admin-password "$ADMIN_PW" \
  >"$WORK/server.log" 2>&1 &
SRV_PID=$!
